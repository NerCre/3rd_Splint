...
(() => {
  ...
  // SMS 送信用の URI を生成（iOS/Android 両対応を狙って ?&body= を採用）
  function smsLink(to, body) {
    const list = (to || []).filter(Boolean).join(',');
    const encoded = encodeURIComponent(body || '');
    // iOS Safari: sms:NUM?&body=... が安定しやすい
    return list ? `sms:${list}?&body=${encoded}` : `sms:?&body=${encoded}`;
  }

  function smsTextFromPreview(p) {
  ...
  function defaultMaster() {
    return {
      ...
      globalContacts: {
        ...
        rescueTeam: '090-0000-0001',
        ambulanceCenter: '090-0000-0002',
      },
      // 送信先範囲（マスタでON/OFF）
      sendScope: {
        safetyHQ: true,
      ...
      companies: [
        { id: 'own', name: '自社', emails: ['090-0000-0010', '090-0000-0011'] },
        { id: 'a', name: 'A造船', emails: ['090-0000-0020', '090-0000-0021'] },
        { id: 'b', name: 'B株式会社', emails: ['090-0000-0030'] },
      ],
      locations: [
  ...
  function buildWizardPreview() {
    ...
    lines.push('');
    lines.push('—');
    lines.push('※このSMSは「命をツナゲル」から作成されました（未確定項目を含む場合があります）。');
  ...
  function renderWizardReview() {
    ...
    // Note: Actual sending happens via "SMSを開く" / "内容をコピー".
  }
  ...
  async function copyPreview() {
    const text =
      `送信先: ${state.preview.to.join(', ')}\n` +
      `SMS本文:\n${smsTextFromPreview(state.preview)}`;
    try {
  ...
  function openSms() {
    const href = smsLink(state.preview?.to || [], smsTextFromPreview(state.preview));
  ...
  function wireGlobalEvents() {
    ...
    $('#btnOpenMail').addEventListener('click', () => openSms());
    $('#btnOpenMailEmergency')?.addEventListener('click', () => openSms());
